  ...
  const [formData, setFormData] = useState({
    return (
      <>
        ...
        ) : loading && (!id || expediteurs.length === 0) ? (
          ...
          <div className="max-w-lg mx-auto px-4 py-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h1 className="text-2xl font-bold">
                  {id ? "Modifier un colis" : "Saisie rapide d'un colis"}
                </h1>
                <p className="text-muted-foreground text-sm">
                  Remplis uniquement les informations essentielles
                </p>
              </div>
            </div>
            <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-lg shadow p-6">
              ...
              <div className="flex justify-end gap-2 mt-4">
                <Button type="submit" className="bg-green-600 hover:bg-green-700 text-white px-8 py-2 rounded-lg text-lg shadow">
                  {loading ? "Enregistrement..." : id ? "Mettre à jour" : "Enregistrer"}
                </Button>
              </div>
            </form>
          </div>
